...
smooth_log_periodicity.pi_chord <- function(x, d = 0.011) {
  ...
  mean(vapply(seq_along(x), function(i) {
    tmp_chord <- x - x[i]
    log2(relative_periodicity(stolz15_rationalise_chord(tmp_chord, d = d)))
  }, numeric(1)))
}
...
stolz15_fraction <- function(x, d, verbose = FALSE) {
  x_min <- (1 - d) * x
  x_max <- (1 + d) * x
  a_l <- floor(x)
  b_l <- 1
...
get_rational_interval <- function(x, d) {
  ...
  octave <- floor(x / 12)
  pitch_class <- x %% 12
  res <- stolz15_fraction(2 ^ (pitch_class / 12), d = 0.011)
  while (octave != 0) {
    if (octave < 0) {
